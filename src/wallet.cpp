...
// DigiCube: optional setting to unlock wallet for block minting only;
...
// DigiCube: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // DigiCube: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                // DigiCube: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // DigiCube: not avatar mode
                        {
...
// DigiCube: create coin stake transaction
...
// DigiCube: check 'spent' consistency between wallet and txindex
// DigiCube: fix wallet spent state according to txindex
...
// DigiCube: disable transaction (only for coinstake)
